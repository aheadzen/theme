...
<!--	<p><?php _e( 'Your avatar will be used on your profile and throughout the site. If there is a <a href="http://gravatar.com">Gravatar</a> associated with your account email we will use that, or you can upload an image from your computer.', 'buddypress') ?></p> -->
	...
	<form action="" method="post" class="standard-form" id="avatar-upload-form" enctype="multipart/form-data">
		...
		<?php if ( 'upload-image' == bp_get_avatar_admin_step() ) : ?>
			...
			<p id="avatar-upload">
				<div class="submit">
					<label for="file">Step 1 - Select Photo</label>
					<input type="file" name="file" id="file" />
					<label for="upload">Step 2 - Upload</label>
					<input type="submit" name="upload" id="upload" value="<?php _e( 'Upload Image', 'buddypress' ) ?>" />
					<input type="hidden" name="action" id="action" value="bp_avatar_upload" />
				</div>
			</p>

			<?php if ( bp_get_user_has_avatar() && !strpos( bp_get_loggedin_user_avatar( array( 'html' => false ) ), 'mystery-man' ) ) : ?>
			<h3><?php _e( 'Current Avatar', 'buddypress' ) ?></h3>
			<p><?php bp_loggedin_user_avatar( 'type=full' ); ?></p>
<!--				<p><?php _e( "If you'd like to delete your current avatar but not upload a new one, please use the delete avatar button.", 'buddypress' ) ?></p> -->
				<p>
					<a class="button edit" href="<?php bp_avatar_delete_link() ?>" title="<?php _e( 'Delete Avatar', 'buddypress' ) ?>"><?php _e( 'Delete Current Avatar', 'buddypress' ) ?></a>
				</p>
			<?php endif; ?>
		...
		<?php if ( 'crop-image' == bp_get_avatar_admin_step() ) : ?>
			...
			<h3><?php _e( 'Preview and Save Your New Avatar', 'buddypress' ) ?></h3>
			<div class="submit">
				<input type="submit" name="avatar-crop-submit" id="avatar-crop-submit" value="<?php _e( 'Save Image &raquo;', 'buddypress' ) ?>" />
			</div>
