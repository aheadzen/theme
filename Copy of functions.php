...
define('SIGN_COMPATIBILITY',1245); //442
 ...
 function get_breadcrumb( $btext = '' ) {
	...
	if(function_exists('bp_is_group') &&  bp_is_group() )
	{
		...
		$btext = '';
	} else if( function_exists('bp_is_user') && bp_is_user() )
	{
		$btext = '';
		...
		} else if( bp_is_user_forums() )
		{
			...
			$bc[] = $bc_template;
		} else if( function_exists('bp_is_user_settings') && bp_is_user_settings() )
		{
	...
	} else if ( function_exists('bp_is_directory') &&  bp_is_directory() ) {
		$btext = '';
		$bc_template['href'] = '';
		$bc_template['title'] = '';
	...
	if( function_exists('bp_is_blog_page') &&  empty($btext) && bp_is_blog_page() )
		wp_title();
	else echo $btext;
}
...
function pageComments( $comment, $args, $depth )
{
	...
	?>
	  ...
	  <strong><cite id="comments"> 
		...
		</cite></strong> (<?php echo my_comment_author_user_reply($comment->user_id); ?>) :
		<a href="#comment-<?php comment_ID(); ?>" class="alignright comment-permalink" title="Permanent Link to this Comment"><?php comment_date('F jS, Y') ?></a>
...
add_filter( 'get_previous_post_where', 'my_get_previous_post_where' );
add_filter( 'get_next_post_where', 'my_get_next_post_where' );
add_filter( 'threewp_activity_monitor_list_activities', 'my_threewp_list_activities' );
add_action( 'wp_footer', 'my_threewp_log_page_visits' );
if(class_exists('group_chat_activate_extension')){
	add_action( 'init', 'group_chat_activate_extension' );
}
add_action('admin_init', 'prevent_admin_access', 0);
add_filter('request', 'my_birthday_permalinkfix');
add_action ( 'wp_print_styles' , 'my_deregister_style', 100 );
add_action( 'wp_print_scripts', 'my_deregister_javascript', 100 );
remove_action( 'wp_head', 'bp_album_add_css');
add_action( 'wp_head', 'my_album_add_css', 100 );
add_action( 'bp_core_activated_user', 'my_core_activated_user' );
add_action( 'xprofile_updated_profile', 'my_xprofile_updated_profile', 10, 3 );
add_action( 'wp_head', 'my_rel_canonical' );
add_action( 'wp_head', 'my_meta_robots' );
add_filter('xprofile_data_before_save', 'my_xprofile_data_value_before_save');
add_filter('loginout', 'my_loginout');
add_filter('aioseop_title_page', 'my_aioseop_title');
add_filter('aioseop_description', 'my_aioseop_description');
add_action('xprofile_screen_change_avatar', 'my_core_add_jquery_cropper');
add_action('groups_screen_group_admin_avatar', 'my_core_add_jquery_cropper');
add_action('template_notices', 'userhome_notifications');
add_action('send_email_subscriptions_cron', 'send_email_subscriptions');
remove_action( 'wp_head', 'bp_core_add_ajax_url_js' );
remove_action( 'wp_head', 'bp_core_confirmation_js', 100 );
remove_action( 'wp_enqueue_scripts', 'bp_follow_add_js' );
